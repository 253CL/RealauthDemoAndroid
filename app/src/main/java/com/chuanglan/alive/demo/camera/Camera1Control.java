...
public class Camera1Control implements ICameraControl {
    ...
    private void startAutoFocus() {
        CameraTimerTask.createAutoFocusTimerTask(() -> {
            synchronized (Camera1Control.this) {
                if (camera != null && !takingPicture.get()) {
                    try {
                        ...
                        // startPreview 是异步实现，可能在某些机器上前几次调用会 autofocus failed
                    }
                }
            }
        });
    ...
    private Comparator<Camera.Size> sizeComparator = (lhs, rhs) ->
            Long.signum((long) lhs.width * lhs.height - (long) rhs.width * rhs.height);
    ...
    /**
     * 有些相机匹配不到完美的比例，比如，我们的 layout 是 4:3 的，预览只有 16:9 的，如果直接显示图片会拉伸变形，
     * 缩放的话，又有黑边，所以我们采取的策略是，等比例放大，这样预览的有一部分会超出屏幕，拍照后再进行裁剪处理
     */
