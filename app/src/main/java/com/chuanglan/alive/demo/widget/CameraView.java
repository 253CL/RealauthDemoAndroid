...
import com.chuanglan.alive.demo.camera.Camera1Control;
...
public class CameraView extends FrameLayout {
    ...
    private final CameraViewTakePictureCallback cameraViewTakePictureCallback =
            new CameraViewTakePictureCallback();
    ...
    public void setMaskType(@MaskView.MaskType int maskType, final Context ctx) {
        ...
        hintViewText.setText(getTipsMessage());

        int hintResourceId = R.drawable.ocr_hint_align_id_card;
        boolean isNeedSetImage = true;

        switch (maskType) {
            case MaskView.MASK_TYPE_ID_CARD_FRONT:
                hintResourceId = R.drawable.ocr_round_corner;
            ...
            case MaskView.MASK_TYPE_BANK_CARD:
                hintResourceId = R.drawable.ocr_hint_align_bank_card;
                break;
            case MaskView.MASK_TYPE_PASSPORT:
                hintView.setVisibility(INVISIBLE);
                break;
            case MaskView.MASK_TYPE_GENERAL:
                break;
    ...
    public void setPreviewMarginTop(int margin) {
        maskView.setPreviewMarginTop(margin);
    }

    public void setMaskColor(int maskColor) {
        maskView.setMaskColor(maskColor);
    }

    public void setLineColor(int lineColor) {
        maskView.setLineColor(lineColor);
    }

    private String getTipsMessage() {
        String message = "请将身份证完整置于取景框内";
        if (maskType == MaskView.MASK_TYPE_ID_CARD_FRONT) {
            message = getContext().getString(R.string.online_alive_detect_card_ocr_front_tip);
        } else if (maskType == MaskView.MASK_TYPE_ID_CARD_BACK) {
            message = getContext().getString(R.string.online_alive_detect_card_ocr_back_tip);
        }
        return message;
    }
    ...
    private void init() {
        ...
        hintViewText.setPadding(ScreenUtils.dpToPx(10), 0, ScreenUtils.dpToPx(10), 0);
        hintViewText.setGravity(Gravity.CENTER);
        hintViewText.setTextColor(Color.parseColor("#FF606581"));
        hintViewText.setTextSize(TypedValue.COMPLEX_UNIT_SP, 14);
        hintViewText.setTypeface(Typeface.DEFAULT, Typeface.BOLD);
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        int hintViewWidth = ScreenUtils.dpToPx(320);
        int hintViewHeight = ScreenUtils.dpToPx(25);

        int hintViewLeft = (getWidth() - hintViewWidth) / 2;
        int hintViewTop = maskView.getFrameRect().bottom + ScreenUtils.dpToPx(16);
    ...
    private Bitmap crop(File outputFile, byte[] data, int rotation) {
        try {
            ...
            // 如果是 1280* 720 的屏幕，裁剪位置下移 46dp(20210713)
            if (ScreenUtils.is1280x720(getContext())) {
                int addTop = ScreenUtils.dpToPx(46);
                region.top = top + addTop;
                region.bottom = bottom + addTop;
            } else {
                region.top = top;
                region.bottom = bottom;
            }
    ...
    private class CameraViewTakePictureCallback implements ICameraControl.OnTakePictureCallback {
        ...
        public void onPictureTaken(final byte[] data) {
            new Thread() {
                @Override
                public void run() {
                    final int rotation = ImageUtils.getOrientation(data);
